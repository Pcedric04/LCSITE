...
class postsController extends Controller
{
    ...
    public function index()
    {
        ...
        $posts = DB::table('categories')
            ...
            ->paginate(6);
        return view('Layouts.LayoutsFrontApp.Actualites.allactualites', ['message' => $message, 'posts' => $posts,'postsAll'=>$postall,'agendas'=>$agendas]);
    }
    public function souscatActu($id)
    {
        ...
        $posts = DB::table('categories')
            ...
            ->paginate(6);
        return view('Layouts.LayoutsFrontApp.Actualites.actualitessouscat', ['message' => $message, 'posts' => $posts,'postsAll'=>$postall,'agendas'=>$agendas]);
    }
    public function details(posts $post,$id)
    {
        ...
        $Comment_Post = DB::table('comments')
                        ...
                        ->where('comments.status','=','En ligne')
                        ->select(
                            'comments.id',
                            'posts.id as Pid',
                            'posts.titre',
...
$postsall1 = DB::table('posts')->orderByDesc('id')
        ...
        ->where('id','!=',$id )
        ->paginate(3);
        return view('Layouts.LayoutsFrontApp.Actualites.oneactualitesdetails',
                [/* 'visitCount'=>$visitCount, */'postsall1'=>$postsall1, 'posts' => $posts, 'post_all'=>$post_all, 'message' => $message,'sousCat'=>$sousCat,'Comment_Post'=>$Comment_Post,'agendas'=>$agendas]);
    }
