...
class albumsController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return dataTables()->of($albums)->addColumn('actions', function ($albums) {
            return view('backoffice.body.Albums.Actions.actions', ['albums' => $albums]);
        })->toJson();
    }
    public function indexAlbums()
    {
        ...
        return view('backoffice.body.Albums.index',compact('photo'));
    }

    public function createAlbums()
    {
        $categories = categories::where('status', '=', 'En ligne')->get();
        return view('backoffice.body.Albums.Modals.create', compact('categories'));
    }
    ...
    public function storeAlbums(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            ...
            request()->logo->move(public_path('front/admin/albums'), $present1);
        ...
        if ($request->hasFile('albums')) {
            foreach($request->file('albums') as $key => $file)
                ...
                $paths= $file->move(public_path('front/admin/albums_single'), $name);
                $file = new photos();
        ...
        return redirect()->back()->with('success',"Fichier télechargé!");
    }
