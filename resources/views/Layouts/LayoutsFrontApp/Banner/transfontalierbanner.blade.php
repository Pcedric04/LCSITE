<!-- Banniere Start -->
<div class="container-fluid page-header py-5 mb-2">
    <div class="container text-center py-5">
        <h1 class="display-2 text-white mb-4">Transfrontalier et migration</h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb justify-content-center mb-0">
                <li class="breadcrumb-item"><a href="{{ route('labo.index') }}">Accueil</a></li>
                <li class="breadcrumb-item"><a href="#">Programmes</a></li>
                <li class="breadcrumb-item text-primary" aria-current="page">Transfrontalier et migration</li>
            </ol>
        </nav>
    </div>
</div>
<!-- Banniere End -->
